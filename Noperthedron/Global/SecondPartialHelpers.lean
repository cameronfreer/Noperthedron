...
Five compositional `@[fun_prop]` core lemmas prove joint differentiability of each
rotation map once, at the matrix-entry level; the `differentiableAt_*` family below
(used ~30+ times in second_partial_inner_rotM_inner) then follows by `fun_prop`.
-/
